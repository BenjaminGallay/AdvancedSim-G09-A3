...
def get_intersection_df(roads_preprocessed, roads_shp):
    ...
    """
    Given a preprocessed roads DataFrame, returns a DataFrame of crossing rows (intersections)
    to be concatenated to the original roads DataFrame. Works directly with CSV data.
    """
    ...
    # Find intersections between all pairs of roads
    intersection_points = []
    road_names = list(csv_road_lines.keys())
    for i in range(len(road_names)):
        for j in range(i + 1, len(road_names)):
            road1, road2 = road_names[i], road_names[j]
            line1, line2 = csv_road_lines[road1], csv_road_lines[road2]
    ...
    nodes = pd.DataFrame(intersection_points)
    print(f"Identified {len(nodes)} unique intersection points from CSV geometries.")
    ...
    distance_threshold = 0.015 # ~1.5km in degrees (approx, at equator)
    dedup_threshold = 0.015 # ~100m in degrees (approx, at equator)
